...
const LayoutAddArticle = () => {
    ...
    const handleFormSubmit = async () => {
        try {
            ...
            formData.append('content', content); // Sử dụng state content
            formData.append('category', JSON.stringify({ id: values.category }));
    
            // Correcting the tag structure
    ...
    return (
        <div style={{ padding: '10px', margin: 'auto', width: '1000px', backgroundColor: '#fff', borderRadius: '10px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
            ...
            <Form className='container mt-5' labelCol={{ span: 3 }} wrapperCol={{ span: 100 }} layout='horizontal' style={{ width: 900 }} onFinish={handleFormSubmit}>
                ...
                <Form.Item style={{ paddingTop: '30px' }} label='Upload' name='file' valuePropName='fileList' getValueFromEvent={normFile} rules={[{ required: true, message: 'Vui lòng thêm ảnh!' }]}>
